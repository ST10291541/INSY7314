...
const AdminPayments = () => {
  ...
  const handleApprove = async (paymentId) => {
    const result = await Swal.fire({
      ...
      text: 'Are you sure you want to approve this payment? Once approved, payments cannot be reversed!',
      icon: 'warning',
    ...
    try {
      ...
      setPayments(prevPayments => 
        prevPayments.map(payment => 
          payment._id === paymentId ? response.data.payment : payment
        )
      );
      ...
      const errorMessage = err.response?.data?.message || 'Failed to approve payment. Please try again.';
      await Swal.fire({
        title: 'Error',
        text: errorMessage,
        icon: 'error',
  ...
  const handleReject = async (paymentId) => {
    ...
    if (rejectionReason === undefined) return; // User cancelled input
    ...
    try {
      ...
      const errorMessage = err.response?.data?.message || 'Failed to reject payment. Please try again.';
      await Swal.fire({
        title: 'Error',
        text: errorMessage,
        icon: 'error',
                ...
                <tr key={payment._id}>
                  <td>
                    <div className="customer-info">
                      ...
                      <small>{payment.customerEmail}</small><br />
                      <small>Acc: {payment.customerAccount}</small>
                    </div>
                  </td>
                  <td>
                    <div className="payee-info">
                      <strong>{payment.payeeName}</strong><br />
                      <small>Acc: {payment.payeeAccountNumber}</small>
                    </div>
                  </td>
                  <td>
                    <div className="bank-info">
                      <strong>{payment.payeeBankName}</strong><br />
                      <span className="swift-code">{payment.swiftCode}</span>
                    </div>
                  </td>
                  <td className="amount-cell">{payment.amount} {payment.currency}</td>
                  <td>
                    <div className="reference-info">
                      <small><strong>Payer:</strong> {payment.paymentReference}</small>
                      {payment.payeeRef && (<><br /><small><strong>Payee:</strong> {payment.payeeRef}</small></>)}
                    </div>
                  </td>
                  <td><span className={`status ${payment.status}`}>{payment.status}</span></td>
                  <td>{new Date(payment.createdAt).toLocaleDateString()}<br /><small>{new Date(payment.createdAt).toLocaleTimeString()}</small></td>
                  <td className="actions">
                    {payment.status === 'pending' ? (
                      ...
                      <span className="no-actions">
                        {payment.status === 'verified' ? 'Approved' : payment.status === 'rejected' ? 'Rejected' : 'No actions'}
                      </span>
                    )}
                  </td>
                </tr>
